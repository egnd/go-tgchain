...
docker-benchmarks:
	docker run --rm -it -v $$(pwd):/src -w /src --entrypoint make golang:1.17 benchmarks

docker-profiling:
	docker run --rm -it -v $$(pwd):/src -w /src --entrypoint sh golang:1.17 -c "apt-get update && apt-get install -y graphviz && make profiling"
	@echo "Look at file://$$(pwd)/coverage/cpu.svg"
	@echo "Look at file://$$(pwd)/coverage/mem.svg"

docker-vendor:
	docker run --rm -it -v $$(pwd):/src -w /src --entrypoint go golang:1.17 mod vendor